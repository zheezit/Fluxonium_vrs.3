...
path = r"C:\Users\jiaop\OneDrive\Skrivebord\Fluxonium\python\Designs_gds"
filename = "no more examples"
dt = datetime.now()
...
if Output:
    if Status == 'fabrication':
        ...
        if dxf:
            dxf_file_name = os.path.join(path, '{:%Y.%m.%d_%H.%M.%S}_{}_fab.dxf'.format(dt, filename))
